...
-- 先删除所有旧表（CASCADE 处理外键依赖），确保每次重建为最新结构
DROP TABLE IF EXISTS campus.token_blacklist CASCADE;
DROP TABLE IF EXISTS campus.notices CASCADE;
DROP TABLE IF EXISTS campus.scholarship_records CASCADE;
DROP TABLE IF EXISTS campus.repair_orders CASCADE;
DROP TABLE IF EXISTS campus.student_schedule CASCADE;
DROP TABLE IF EXISTS campus.schedules CASCADE;
DROP TABLE IF EXISTS campus.admins CASCADE;
DROP TABLE IF EXISTS campus.students CASCADE;
...
-- --------------------------
CREATE TABLE campus.student_schedule (
...
-- 宿舍报修工单
-- --------------------------
CREATE TABLE campus.repair_orders (
    id              SERIAL PRIMARY KEY,
    student_id      INT          NOT NULL REFERENCES campus.students(id) ON DELETE CASCADE,
...
-- 奖助记录
-- --------------------------
CREATE TABLE campus.scholarship_records (
    id              SERIAL PRIMARY KEY,
    student_id      INT          NOT NULL REFERENCES campus.students(id) ON DELETE CASCADE,
...
-- --------------------------
CREATE TABLE campus.token_blacklist (
    id          SERIAL PRIMARY KEY,
...
-- 先删除旧表，确保每次重建为最新结构
DROP TABLE IF EXISTS ai_campus.messages CASCADE;
DROP TABLE IF EXISTS ai_campus.conversations CASCADE;
DROP TABLE IF EXISTS ai_campus.faq_embeddings CASCADE;

-- --------------------------
-- FAQ 向量嵌入表（pgvector）
-- 存储校园 FAQ 文档的分块向量，用于 RAG 检索
-- --------------------------
CREATE TABLE ai_campus.faq_embeddings (
    id          SERIAL PRIMARY KEY,
...
-- 会话表
-- --------------------------
CREATE TABLE ai_campus.conversations (
    id          SERIAL PRIMARY KEY,
    student_id  INT          NOT NULL REFERENCES campus.students(id) ON DELETE CASCADE,
...
-- 消息表
-- --------------------------
CREATE TABLE ai_campus.messages (
    id              SERIAL PRIMARY KEY,
...
CREATE INDEX idx_messages_conversation ON ai_campus.messages(conversation_id, created_at);
