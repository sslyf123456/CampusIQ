...
INTENT_CATEGORIES = {
    ...
    "student": "学生信息查询 — 用户询问学生名单、学生详情、院系学生、学生联系方式等信息",
...
- semester: 如果意图为schedule且用户提到了学期（"本学期""这学期""上学期""2025-2026-2" 等），请输出学期字符串（如 "2025-2026-2"）；用户没提学期或与学期无关则为null

学期字符串格式："起始学年-结束学年-1或2"，例如 "2025-2026-2" 表示 2025-2026 学年的第二学期。
"本学期/这学期/当前学期" 应当按系统提示词中"当前学期"字段填充；用户明确提到"上学期/2024-2025-1"等具体学期时按用户表述填充。
...
class MasterAgent:
    ...
    async def identify_intent(self, user_message: str, history_context: str = "", role: str = "student") -> IntentResult:
        ...
        try:
            ...
            semester = self._extract_semester(parsed, user_message)
            return IntentResult(
    ...
    @staticmethod
    def _extract_semester(parsed: dict, user_message: str) -> Optional[str]:
        """从 LLM 输出或用户消息中提取学期字符串。

        优先级：
        1. LLM 显式输出的 semester 字段
        2. 用户消息中显式提到的学期字符串（"2025-2026-2" 格式）
        3. 用户消息中出现"本学期/这学期/当前学期"时，按系统时间推算当前学期
        """
        ...
        # 用户消息中显式提到的学期
        match = re.search(r"\b(\d{4}-\d{4}-[12])\b", user_message)
        if match:
            return match.group(1)

        # "本学期/这学期/当前学期" 兜底
        if any(kw in user_message for kw in ["本学期", "这学期", "当前学期", "这学期的", "本学期的"]):
            from app.prompts import _get_current_semester
            from datetime import datetime
            return _get_current_semester(datetime.now())
    ...
    async def run(self, query: str, history_context: str = "", **kwargs) -> IntentResult:
        ...
        Args:
            query: 用户查询内容
            history_context: 对话历史上下文（可选），用于理解上下文关联的提问
            **kwargs: 包含 role 的关键字参数

        Returns:
            IntentResult: 意图识别结果
        """
        role = kwargs.get("role", "student")
        return await self.identify_intent(query, history_context=history_context, role=role)
